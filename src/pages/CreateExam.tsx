...
import { getAdminSession, createExam } from '@/lib/store';
...
const CreateExam = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { toast.error('Enter exam name'); return; }
    ...
    try {
      const exam = await createExam(name.trim(), settings, startDateTime, endDateTime);
      if (exam) {
        toast.success(`Exam created! Code: ${exam.code}`);
    ...
    <div className="min-h-screen pt-20 bg-background">
      ...
      <main className="container max-w-2xl py-8 animate-fade-in">
        <form onSubmit={handleCreate} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input placeholder="e.g. Mathematics Mid-Term" value={name} onChange={e => setName(e.target.value)} required />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label>Duration (minutes)</Label>
                  <Input type="number" min={1} value={settings.duration} onChange={e => updateSetting('duration', Number(e.target.value))} />
                </div>
                <div className="space-y-2">
                  <Label>Marks per Question</Label>
                  <Input type="number" min={1} value={settings.marksPerQuestion} onChange={e => updateSetting('marksPerQuestion', Number(e.target.value))} />
                </div>
                <div className="space-y-2">
                  <Label>Negative Marks</Label>
                  <Input type="number" min={0} step={0.25} value={settings.negativeMarks} onChange={e => updateSetting('negativeMarks', Number(e.target.value))} />
                </div>
                <div className="space-y-2">
                  <Label>Question Timer (sec, 0=off)</Label>
                  <Input type="number" min={0} value={settings.questionTimer} onChange={e => updateSetting('questionTimer', Number(e.target.value))} />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              {[
                ...
                <div key={key} className="flex items-center justify-between">
                  ...
                  <Switch checked={settings[key] as boolean} onCheckedChange={v => updateSetting(key, v)} />
                </div>
              ))}
              {settings.tabSwitchDetection && (
                <div className="space-y-2">
                  <Label>Max Tab Switches (auto-submit after)</Label>
                  <Input type="number" min={1} value={settings.maxTabSwitches} onChange={e => updateSetting('maxTabSwitches', Number(e.target.value))} />
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Button type="submit" className="w-full" size="lg">Create Exam</Button>
        </form>
      </main>
    </div>
  );
